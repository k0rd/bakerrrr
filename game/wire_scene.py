"""Bounded local WireScene projection for Slice 3 hacking traversal."""
...
def _scene_id(sim, actor_eid, connection, target_prop, interface):
    seed = _clean_text(getattr(sim, "seed", ""), "seed")
    return ":".join((
        "wire",
        str(seed),
        str(actor_eid),
        _clean_text((target_prop or {}).get("id")),
...
def build_wire_scene(sim, actor_eid, prop, *, item_catalog=None):
    ...
    if _clean_text(connection.get("target_property_id")) != target_property_id:
    ...
    target_name = _prop_name(prop, target_class.replace("_", " ") or "wire target")
    metadata = _prop_metadata(prop)
    linked_id = _clean_text(connection.get("linked_property_id") or metadata.get("linked_property_id"))
    ...
    if target_class == "service_terminal" and is_atm_surface:
        ...
        branch_read = [
            ...
            "The index is readable, but no service action executes in this slice.",
        ]
    else:
        ...
        branch_read = [
            ...
            "Door, alarm, and camera consequences are deferred until physical consequence slices.",
        ]
    ...
    nodes = [
        ...
        _node(
            ...
            [
                ...
                f"Security read: tier {security}; warnings are informational in Slice 3.",
            ],
            glyph="?",
        ),
        _node(
            ...
            [
                ...
                "No controller command is executable yet.",
            ],
            glyph="C",
        ),
    ...
    scene = {
        ...
        "scene_id": _scene_id(sim, actor_eid, connection, prop, interface),
    ...
    seed_wire_users_for_scene(scene, sim=sim, source_prop=prop, linked_prop=linked_prop)
    return {"ok": True, "reason": None, "scene": scene}


def open_wire_scene(sim, actor_eid, prop, *, item_catalog=None):
    state = wire_state_for_actor(sim, actor_eid, create=True)
    result = build_wire_scene(sim, actor_eid, prop, item_catalog=item_catalog)
...
def _scene_stale_reason(sim, actor_eid, scene):
    ...
    if _clean_text(active.get("target_property_id")) != _clean_text(scene.get("target_property_id")):
