...
def prime_tutorial_final_operation(
    sim,
    player_eid,
    target_property_id,
    target_item_instance_id=None,
    ...
    del player_eid
    state = _state(sim)
    prop = sim.properties.get(str(target_property_id)) if sim is not None else None
    if not isinstance(prop, dict):
        return None

    prop_x = _safe_int(prop.get("x"), default=0)
    prop_y = _safe_int(prop.get("y"), default=0)
    prop_z = _safe_int(prop.get("z"), default=0)
    target_chunk = _property_chunk(sim, prop) or sim.chunk_coords(prop_x, prop_y)
    item_id = _text(target_item_id).lower() or "access_badge"
    instance_id = _text(target_item_instance_id) or sim.new_item_instance_id()
    ground_id = _text(target_ground_item_id)
    ground = sim.ground_items.get(ground_id) if ground_id else None
    if not isinstance(ground, dict):
        metadata = {
            "display_name": _text(target_item_name) or item_display_name(item_id),
            "tutorial": True,
        ...
        ground = sim.ground_items.get(ground_id)
    if isinstance(ground, dict):
        ground_meta = ground.get("metadata")
        if not isinstance(ground_meta, dict):
            ground_meta = {}
            ground["metadata"] = ground_meta
        ground_meta.update({
            "final_operation_target": True,
            "final_operation_objective_id": "high_value_retrieval",
            "final_operation_property_id": prop.get("id"),
            "final_operation_target_label": _text(target_label) or "tutorial block",
            "final_operation_target_chunk": tuple(target_chunk),
            "tutorial": True,
            "claim_class": "public_free",
            "source_context": "tutorial",
        })
        item_id = _text(ground.get("item_id")).lower() or item_id
        instance_id = _text(ground.get("instance_id")) or instance_id
        target_item_name = _text(target_item_name) or item_display_name(item_id, metadata=ground_meta)

    state.update({
        "unlocked": True,
        "completed": False,
        "failed": False,
        "unlock_tick": _safe_int(getattr(sim, "tick", 0), default=0),
        "complete_tick": -1,
        "fail_tick": -1,
        "fail_reason": "",
        "objective_id": "high_value_retrieval",
        "objective_title": "Tutorial Retrieval",
        "target_chunk": tuple(target_chunk),
        "target_label": _text(target_label) or "tutorial block",
        "target_property_id": _text(prop.get("id")),
        "target_property_name": _text(prop.get("name")) or "Tutorial Retrieval Locker",
        "target_item_id": item_id,
        "target_item_name": _text(target_item_name) or item_display_name(item_id),
        "target_item_instance_id": instance_id,
        "target_ground_item_id": ground_id,
        "target_recovered": False,
        "target_identified_tick": _safe_int(getattr(sim, "tick", 0), default=0),
        "target_recovered_tick": -1,
        "target_reason": "tutorial placement",
        "target_quality_label": "training",
        "target_value_bonus": 0,
        "target_intel_score": 0,
        "target_entry_label": "open street locker",
        "target_entry_detail": "The tutorial target is staged in a visible locker inside the training block.",
        "summary": {},
    })
    return {
        ...
        "target_item_instance_id": state.get("target_item_instance_id", ""),
        "target_ground_item_id": state.get("target_ground_item_id", ""),
    }
