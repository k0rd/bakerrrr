...
NPC_COMMUTE_DRIVER_CAP_PER_CHUNK = 4
...
def _ambient_animal_profile(
    ...
    *,
    ...
    profile = {
        ...
        "areas": dict(areas),
        "districts": dict(districts or {}),
        "terrains": dict(terrains),
        "spawn_zones": tuple(spawn_zones),
        "archetypes": tuple(archetypes),
        "color": str(color),
        "max_hp": tuple(max_hp),
        "speed": tuple(speed),
        "noise_radius": int(noise_radius),
        "home_radius": int(home_radius),
        "flee_radius": int(flee_radius),
        "flock_radius": int(flock_radius),
        "flocking": bool(flocking),
        "activity_period": str(activity_period),
        "rest_bias": float(rest_bias),
        "threat_response": str(threat_response),
        "movement_style": str(movement_style),
        "physical_profile": {"size_score": float(size_score), "speed_score": float(speed_score)},
        "ecology_profile": {
            "species": str(profile_id),
            "predator_score": float(predator_score),
            "prey_score": float(prey_score),
            "scavenger_score": float(scavenger_score),
            "territorial_score": float(territorial_score),
            "pack_score": float(pack_score),
            "flee_bias": float(flee_bias),
            "chase_bias": float(chase_bias),
        },
    }
    if requires_water_edge:
        profile["requires_water_edge"] = True
    if isinstance(animal_context, dict):
        profile["animal_context"] = dict(animal_context)
    if isinstance(social_profile, dict):
        profile["social_profile"] = dict(social_profile)
    if genetic_patterns:
...
AMBIENT_CREATURE_PROFILES = (
    ...
    _ambient_animal_profile(
        "eastern_cottontail", "other", "sylvilagus floridanus",
        ("cottontail rabbit", "brush rabbit", "yard cottontail"),
        areas={"frontier": 1.05, "wilderness": 0.82, "city": 0.18},
        districts={"residential": 0.1}, terrains={"scrub": 0.24, "plains": 0.2, "park": 0.18, "forest": 0.08},
        archetypes=("house", "field_camp", "ranger_hut", "work_shed"), color="other",
        max_hp=(12, 20), speed=(1.08, 1.3), size_score=12, speed_score=76,
        prey_score=72, pack_score=34, flee_bias=92, home_radius=4, flee_radius=7,
        flocking=True, activity_period="crepuscular", rest_bias=0.24,
        threat_response="freeze_bolt", movement_style="dart", genetic_patterns=("plain", "dappled", "spotted"),
    ),
    _ambient_animal_profile(
        "snowshoe_hare", "other", "lepus americanus",
        ("snowshoe hare", "forest hare", "white-footed hare"),
        areas={"wilderness": 0.72, "frontier": 0.34}, terrains={"forest": 0.3, "hills": 0.2, "scrub": 0.1},
        archetypes=("ranger_hut", "field_camp", "firewatch_tower", "lookout_post"), color="other",
        max_hp=(13, 21), speed=(1.1, 1.32), size_score=14, speed_score=80,
        prey_score=76, pack_score=26, flee_bias=94, home_radius=5, flee_radius=8,
        activity_period="crepuscular", rest_bias=0.22, threat_response="freeze_bolt", movement_style="dart",
        genetic_patterns=("plain", "dappled", "mottled"),
    ),
    _ambient_animal_profile(
        "gray_squirrel", "rodent", "sciurus carolinensis",
        ("gray squirrel", "park squirrel", "yard squirrel"),
        areas={"city": 0.72, "frontier": 0.86, "wilderness": 0.48},
        districts={"residential": 0.2, "downtown": 0.05}, terrains={"park": 0.34, "forest": 0.26, "urban": 0.06},
        archetypes=("house", "corner_store", "ranger_hut", "field_camp"), color="rodent",
        max_hp=(10, 17), speed=(1.08, 1.3), size_score=7, speed_score=74,
        prey_score=68, scavenger_score=18, pack_score=38, flee_bias=90,
        flocking=True, activity_period="day", rest_bias=0.2, threat_response="freeze_bolt", movement_style="dart",
    ),
    _ambient_animal_profile(
        "fox_squirrel", "rodent", "sciurus niger",
        ("fox squirrel", "rust squirrel", "big yard squirrel"),
        areas={"frontier": 0.66, "wilderness": 0.58, "city": 0.18},
        districts={"residential": 0.08}, terrains={"forest": 0.25, "park": 0.2, "plains": 0.08},
        archetypes=("house", "field_camp", "ranger_hut", "work_shed"), color="rodent",
        max_hp=(11, 19), speed=(1.04, 1.24), size_score=9, speed_score=68,
        prey_score=64, scavenger_score=20, pack_score=32, flee_bias=86,
        activity_period="day", rest_bias=0.22, threat_response="freeze_bolt", movement_style="dart",
    ),
    _ambient_animal_profile(
        "red_fox", "canine", "vulpes vulpes",
        ("red fox", "brush fox", "field fox"),
        areas={"wilderness": 0.46, "frontier": 0.4, "city": 0.04},
        districts={"residential": 0.02}, terrains={"forest": 0.2, "scrub": 0.2, "plains": 0.14, "hills": 0.08},
        archetypes=("field_camp", "ranger_hut", "work_shed", "survey_post"), color="canine",
        max_hp=(24, 36), speed=(1.04, 1.22), size_score=30, speed_score=68,
        predator_score=48, prey_score=24, scavenger_score=36, territorial_score=30, chase_bias=58, flee_bias=48,
        home_radius=7, flee_radius=7, activity_period="crepuscular", rest_bias=0.34,
        threat_response="flee", movement_style="stalk", animal_context={"hunger": 62.0},
    ),
    _ambient_animal_profile(
        "swift_fox", "canine", "vulpes velox",
        ("swift fox", "prairie fox", "sand fox"),
        areas={"frontier": 0.34, "wilderness": 0.3}, terrains={"plains": 0.28, "scrub": 0.2, "badlands": 0.12},
        archetypes=("field_camp", "survey_post", "lookout_post", "truck_stop"), color="canine",
        max_hp=(20, 31), speed=(1.08, 1.28), size_score=25, speed_score=76,
        predator_score=42, prey_score=30, scavenger_score=28, territorial_score=24, chase_bias=62, flee_bias=54,
        home_radius=7, flee_radius=8, activity_period="night", rest_bias=0.3,
        threat_response="flee", movement_style="stalk", animal_context={"hunger": 64.0},
    ),
    _ambient_animal_profile(
        "bobcat", "feline", "lynx rufus",
        ("bobcat", "brush lynx", "red lynx"),
        areas={"wilderness": 0.24, "frontier": 0.12}, terrains={"forest": 0.18, "hills": 0.16, "scrub": 0.14, "marsh": 0.04},
        archetypes=("ranger_hut", "field_camp", "survey_post", "ruin_shelter"), color="feline",
        max_hp=(34, 50), speed=(1.04, 1.2), size_score=45, speed_score=70,
        predator_score=68, prey_score=12, scavenger_score=18, territorial_score=48, chase_bias=72, flee_bias=34,
        home_radius=8, flee_radius=7, activity_period="crepuscular", rest_bias=0.4,
        threat_response="display", movement_style="stalk", animal_context={"hunger": 68.0, "territorial_context": True},
        genetic_patterns=("spotted", "dappled", "mottled"),
    ),
    _ambient_animal_profile(
        "wild_turkey", "avian", "meleagris gallopavo",
        ("wild turkey", "wood turkey", "field turkey"),
        areas={"wilderness": 0.62, "frontier": 0.38}, terrains={"forest": 0.24, "plains": 0.16, "scrub": 0.12},
        archetypes=("ranger_hut", "field_camp", "herbalist_camp", "lookout_post"), color="avian",
        max_hp=(26, 39), speed=(0.98, 1.16), size_score=34, speed_score=54,
        prey_score=58, scavenger_score=12, territorial_score=30, pack_score=72, flee_bias=80,
        flocking=True, flock_radius=6, activity_period="day", rest_bias=0.24,
        threat_response="display", movement_style="amble", genetic_displays=("fan", "crest"),
    ),
    _ambient_animal_profile(
        "bobwhite_quail", "avian", "colinus virginianus",
        ("bobwhite quail", "brush quail", "field quail"),
        areas={"frontier": 0.7, "wilderness": 0.58}, terrains={"scrub": 0.26, "plains": 0.22, "forest": 0.08},
        archetypes=("field_camp", "work_shed", "ranger_hut", "survey_post"), color="avian",
        max_hp=(10, 17), speed=(1.06, 1.28), size_score=7, speed_score=72,
        prey_score=70, pack_score=78, flee_bias=92, flocking=True, flock_radius=6,
        activity_period="day", rest_bias=0.2, threat_response="freeze_bolt", movement_style="dart",
        genetic_patterns=("mottled", "dappled", "striped"),
    ),
    _ambient_animal_profile(
        "box_turtle", "reptile", "terrapene carolina",
        ("box turtle", "wood turtle", "hinge-shell turtle"),
        areas={"wilderness": 0.62, "frontier": 0.36}, terrains={"forest": 0.24, "marsh": 0.18, "park": 0.1},
        archetypes=("ranger_hut", "herbalist_camp", "field_camp", "weather_station"), color="reptile",
        max_hp=(24, 38), speed=(0.62, 0.78), size_score=12, speed_score=12,
        prey_score=34, territorial_score=8, flee_bias=42, home_radius=3, flee_radius=3,
        activity_period="day", rest_bias=0.58, threat_response="brace", movement_style="amble",
        genetic_patterns=("spotted", "mottled", "dappled"), genetic_displays=("plates",),
    ),
    _ambient_animal_profile(
        "snapping_turtle", "reptile", "chelydra serpentina",
        ("snapping turtle", "mud turtle", "river snapper"),
        areas={"coastal": 0.32, "wilderness": 0.28, "frontier": 0.08},
        terrains={"marsh": 0.3, "lake": 0.22, "waterway": 0.2, "shore": 0.08},
        archetypes=("tide_station", "bait_shop", "dock_shack", "herbalist_camp"), color="reptile",
        max_hp=(34, 52), speed=(0.66, 0.84), size_score=28, speed_score=18,
        predator_score=24, prey_score=22, territorial_score=56, flee_bias=22, chase_bias=16,
        home_radius=3, flee_radius=3, activity_period="crepuscular", rest_bias=0.56,
        threat_response="brace", movement_style="amble", requires_water_edge=True,
        animal_context={"hunger": 48.0, "territorial_context": True}, genetic_displays=("plates",),
    ),
    _ambient_animal_profile(
        "striped_skunk", "other", "mephitis mephitis",
        ("striped skunk", "yard skunk", "brush skunk"),
        areas={"frontier": 0.54, "wilderness": 0.42, "city": 0.12},
        districts={"residential": 0.05}, terrains={"scrub": 0.2, "forest": 0.14, "park": 0.1, "urban": 0.03},
        archetypes=("house", "field_camp", "truck_stop", "salvage_camp"), color="other",
        max_hp=(20, 31), speed=(0.88, 1.04), size_score=22, speed_score=38,
        prey_score=28, scavenger_score=58, territorial_score=42, flee_bias=44,
        activity_period="night", rest_bias=0.38, threat_response="display", movement_style="amble",
        genetic_patterns=("striped", "warning_bands"), genetic_displays=("fan",),
    ),
    _ambient_animal_profile(
        "porcupine", "other", "erethizon dorsatum",
        ("porcupine", "quillback", "wood porcupine"),
        areas={"wilderness": 0.38, "frontier": 0.16}, terrains={"forest": 0.28, "hills": 0.14, "scrub": 0.06},
        archetypes=("ranger_hut", "field_camp", "firewatch_tower", "herbalist_camp"), color="other",
        max_hp=(32, 48), speed=(0.72, 0.9), size_score=32, speed_score=24,
        prey_score=22, scavenger_score=20, territorial_score=46, flee_bias=34,
        activity_period="night", rest_bias=0.46, threat_response="display", movement_style="amble",
        genetic_displays=("quills",),
    ),
    _ambient_animal_profile(
        "nine_banded_armadillo", "other", "dasypus novemcinctus",
        ("armadillo", "nine-banded armadillo", "road armadillo"),
        areas={"frontier": 0.48, "wilderness": 0.26, "coastal": 0.12},
        terrains={"scrub": 0.24, "plains": 0.16, "marsh": 0.08, "forest": 0.06},
        archetypes=("truck_stop", "field_camp", "work_shed", "survey_post"), color="other",
        max_hp=(28, 43), speed=(0.78, 0.98), size_score=24, speed_score=30,
        prey_score=30, scavenger_score=24, territorial_score=12, flee_bias=52,
        activity_period="night", rest_bias=0.44, threat_response="brace", movement_style="amble",
        genetic_patterns=("warning_bands", "mottled"), genetic_displays=("plates",),
    ),
    _ambient_animal_profile(
        "groundhog", "rodent", "marmota monax",
        ("groundhog", "woodchuck", "field marmot"),
        areas={"frontier": 0.58, "wilderness": 0.46, "city": 0.06},
        districts={"residential": 0.02}, terrains={"plains": 0.24, "scrub": 0.18, "forest": 0.1, "park": 0.08},
        archetypes=("field_camp", "house", "work_shed", "ranger_hut"), color="rodent",
    ...
    _ambient_animal_profile(
        ...
        ("barn owl", "pale owl", "night owl"),
        areas={"frontier": 0.42, "wilderness": 0.36, "city": 0.04},
        districts={"industrial": 0.02}, terrains={"plains": 0.2, "scrub": 0.16, "forest": 0.1, "urban": 0.02},
        archetypes=("work_shed", "field_camp", "ranger_hut", "warehouse", "lookout_post"), color="avian",
        max_hp=(18, 28), speed=(1.08, 1.28), size_score=16, speed_score=78,
        predator_score=46, prey_score=24, territorial_score=28, chase_bias=66, flee_bias=54,
        home_radius=8, flee_radius=8, activity_period="night", rest_bias=0.18,
        threat_response="flee", movement_style="stalk", animal_context={"hunger": 62.0}, genetic_displays=("crest",),
    ),
    _ambient_animal_profile(
        "river_otter", "other", "lontra canadensis",
        ("river otter", "bank otter", "water otter"),
        areas={"coastal": 0.42, "wilderness": 0.28, "frontier": 0.08},
        terrains={"waterway": 0.3, "lake": 0.28, "marsh": 0.18, "shore": 0.12},
        archetypes=("tide_station", "bait_shop", "dock_shack", "ranger_hut", "ferry_post"), color="other",
        max_hp=(28, 42), speed=(1.0, 1.2), size_score=34, speed_score=62,
        predator_score=36, prey_score=28, scavenger_score=24, territorial_score=30, pack_score=48, chase_bias=54, flee_bias=52,
        home_radius=6, flee_radius=7, flocking=True, activity_period="crepuscular", rest_bias=0.24,
        threat_response="flee", movement_style="dart", requires_water_edge=True, animal_context={"hunger": 60.0},
    ),
    _ambient_animal_profile(
        "little_brown_bat", "other", "myotis lucifugus",
        ("little brown bat", "barn bat", "night bat"),
        areas={"frontier": 0.52, "wilderness": 0.44, "city": 0.08},
        districts={"industrial": 0.03}, terrains={"forest": 0.2, "hills": 0.12, "urban": 0.03, "marsh": 0.08},
        archetypes=("work_shed", "warehouse", "ranger_hut", "firewatch_tower", "ruin_shelter"), color="other",
        max_hp=(8, 14), speed=(1.12, 1.34), size_score=3, speed_score=84,
        predator_score=12, prey_score=62, pack_score=76, flee_bias=92, chase_bias=28,
        home_radius=7, flee_radius=8, flock_radius=7, flocking=True, activity_period="night", rest_bias=0.14,
        threat_response="freeze_bolt", movement_style="dart",
    ),
)
