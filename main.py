...
def _doctor_report(argv=None, *, save_dir=None):
    ...
    try:
        load_player_config(config_path=save_root / "player_config.json")
        lines.append("player_config: readable")
    except OSError as exc:
        ok = False
...
def _run_new_game_legacy(view, character_name):
    ...
    active_property_chunk = (sim.active_chunk["cx"], sim.active_chunk["cy"])
    sim.chunk_property_records[active_property_chunk] = list(property_records)
    sim.chunk_property_generation_complete.add(active_property_chunk)
...
def _run_pygame(*, debug_mode=False):
...
def _run_entrypoint(argv=None):
    ...
    run_end = _run_pygame(debug_mode=debug_mode)
